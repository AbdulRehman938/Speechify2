...
const Curtain = ({
  ...
  const handleCurtainItemClick = (item) => {
    if (item === 'AI Voice Generator') {
      navigate('/ai-voice-generator');
    }
            ...
            <div className="w-full animate-slide-up-fade-long flex justify-between items-start">
              <ul className="text-left animate-slide-up-fade-long">
                {items.map((text, idx) => (
                  <li
                    key={idx}
                    onClick={() => handleCurtainItemClick(text)}
                    className="cursor-pointer bg-transparent w-[25rem] text-[1.5rem] px-4 py-1 rounded-md hover:text-[#918f8f] transition-colors duration-300"
                  >
                    {text}
                  </li>
                ))}
              ...
              <ul className="text-left animate-slide-up-fade-long">
                {items.map((text, idx) => (
                  <li
                    key={idx}
                    onClick={() => handleCurtainItemClick(text)}
                    onMouseEnter={() => setHoveredLiIndex(idx)}
                    onMouseLeave={() => setHoveredLiIndex(null)}
                    className="cursor-pointer bg-transparent w-[25rem] text-[1.5rem] px-4 py-1 rounded-md hover:text-[#918f8f] transition-colors duration-300"
                  >
                    {text}
                  </li>
